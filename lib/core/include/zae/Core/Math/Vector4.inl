...
namespace zae {
	...
	namespace std {
	template<typename T>
	struct hash<zae::Vector4<T>> {
		size_t operator()(const zae::Vector4<T> &vector) const noexcept {
			size_t seed = 0;
			zae::Math::HashCombine(seed, vector.x);
			zae::Math::HashCombine(seed, vector.y);
			zae::Math::HashCombine(seed, vector.z);
			zae::Math::HashCombine(seed, vector.w);
			return seed;
		}
	};
}