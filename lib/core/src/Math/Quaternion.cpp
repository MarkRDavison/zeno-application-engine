#include <zae/Core/Math/Quaternion.hpp>
...
namespace zae
{
	...
	Quaternion::Quaternion(const Vector3f& source)
	{
		...
		auto cx = Math::CosFromSin(sx, source.x * 0.5f);
		auto sy = std::sin(source.y * 0.5f);
		auto cy = Math::CosFromSin(sy, source.y * 0.5f);
		auto sz = std::sin(source.z * 0.5f);
		auto cz = Math::CosFromSin(sz, source.z * 0.5f);
