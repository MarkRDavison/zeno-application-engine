...
namespace zae
{
	...
	Image::~Image()
	{
		...
		Graphics::CheckVk(vkDeviceWaitIdle(*logicalDevice)); // TODO: Less of a sledgehammer approach
	...
	// TODO: Method name is bad, validate purpose?  Or expose method to set layout member variable value
	void Image::InstanceTransitionImageLayout(const VkImage& image, VkFormat format, VkImageLayout srcImageLayout, VkImageLayout dstImageLayout,
		VkImageAspectFlags imageAspect, uint32_t mipLevels, uint32_t baseMipLevel, uint32_t layerCount, uint32_t baseArrayLayer)
	{
		Image::TransitionImageLayout(image, format, srcImageLayout, dstImageLayout, imageAspect, mipLevels, baseMipLevel, layerCount, baseArrayLayer);
		this->layout = dstImageLayout;
	}
