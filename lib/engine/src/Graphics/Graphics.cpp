...
#include <zae/Core/Utils/Enumerate.hpp>
...
namespace zae
{
	...
	Graphics::Graphics() :
		elapsedPurge(5s),
		instance(std::make_unique<zae::Instance>()),
		...
		logicalDevice(std::make_unique<LogicalDevice>(*instance, *physicalDevice))
	{
		...
		Windows::Get()->OnAddWindow().registerCallback(std::bind(&Graphics::OnAddWindow, this, std::placeholders::_1, std::placeholders::_2));
	...
	void Graphics::OnAddWindow(Window* window, bool added)
	{
