...
namespace zae
{
	Windows* Windows::Instance = nullptr;
	...
	const Window* Windows::GetWindow(WindowId id) const
	{
		...
		{
#ifdef ZAE_DEBUG
			Log::Error("Trying to get a window when the id isn't registered.\n");
			throw std::runtime_error("Trying to get a window when the id isn't registered.");
#endif
			return nullptr;
		}
